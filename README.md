...
- 클립보드 동기화: PC ↔ 브라우저 기기 양방향 (버튼)
- 파일 전송: 공유 폴더 기반 업로드/다운로드 (DataChannel 청크 전송)
- 접속 보호: 토큰 인증
...
- **Linux**: X11 세션 권장. Wayland에서는 화면 캡처/입력이 제한될 수 있습니다.
  클립보드 동기화를 쓰려면 `xclip`(또는 `xsel`)을 설치하세요: `sudo apt install xclip`.
...
| `--files-dir` | `./remote_files` | 파일 전송 공유 폴더. 업로드가 저장되고 다운로드가 제공되는 위치 (환경변수 `FILES_DIR`) |
...
상단 툴바의 버튼으로 텍스트 클립보드를 양방향 전송합니다.

- **📋 PC→여기**: PC의 클립보드를 지금 보고 있는 기기의 클립보드로 가져옵니다.
- **📋 여기→PC**: 이 기기의 클립보드를 PC로 보냅니다. (이후 PC에서 `Ctrl+V`)

제약 사항:

- 브라우저 클립보드 API는 **보안 컨텍스트(HTTPS 또는 localhost)** 에서만 동작합니다.
  cloudflared/ngrok 같은 HTTPS 터널로 접속하면 정상 동작하고, 평문 HTTP(localhost 제외)에서는
  버튼이 표시되지 않거나 실패합니다. 처음 사용할 때 브라우저가 클립보드 권한을 물을 수 있습니다.
- **텍스트만** 지원합니다(이미지·파일 제외).
- 서버 측 클립보드 접근에는 `pyperclip`이 필요하며, **Linux는 `xclip` 또는 `xsel`** 이 설치되어
  있어야 합니다(`sudo apt install xclip`). 사용할 수 없으면 서버가 클립보드 기능을 비활성화하고
  버튼도 숨겨집니다.

### 파일 전송

툴바의 **📁 파일** 버튼으로 패널을 열면 **공유 폴더**(기본 `./remote_files`)를 통해 파일을
주고받을 수 있습니다.

- **⬆ PC로 업로드**: 이 기기의 파일을 골라 PC의 공유 폴더로 전송합니다. (여러 개 선택 가능)
- **⬇ 받기**: 공유 폴더의 파일을 이 기기로 다운로드합니다. **↻ 새로고침**으로 목록을 갱신합니다.

동작 방식과 제약:

- 전송은 WebRTC DataChannel로 16KB씩 청크 단위로 이뤄지며, 진행률이 패널에 표시됩니다.
  채널 버퍼가 차면 자동으로 속도를 조절(back-pressure)합니다.
- 업로드는 항상 공유 폴더 **안**에만 저장되고, 파일명은 basename으로 정규화되어 경로 이탈
  (`../`)이 차단됩니다. 같은 이름이 있으면 `이름 (1).ext` 처럼 자동으로 번호가 붙습니다.
- 다운로드는 **공유 폴더 안의 파일만** 제공됩니다(폴더 밖 접근 불가).
- 대용량 전송 중에는 화면 스트림이 잠깐 끊길 수 있습니다(같은 채널을 사용하기 때문).
...
- 공용 STUN은 IP 발견에만 쓰이며 미디어를 중계하지 않습니다. TURN을 쓸 때는 신뢰할 수 있는
  서버를 사용하세요.
- 파일 전송 공유 폴더(`--files-dir`) 안의 파일은 접속자가 **모두 다운로드**할 수 있고, 접속자가
  올린 파일이 저장됩니다. 민감한 파일이 없는 전용 폴더를 지정하세요(경로 이탈은 차단되지만,
  폴더 자체의 내용은 노출됩니다).
